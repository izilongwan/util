...
export const PageList = ((doc) => {
  var PageList = function (this: ICommonObject, wrap: string, opt: ICommonObject= {}) {
  ...
  PageList.prototype = {
    ...
    onPageListClick: function (e:MouseEvent) {
      var e = e || window.event,
          tar: EventTarget | null = e.target || e.srcElement,
          className = (<HTMLElement>tar).className,
      ...
      if (className) {
        switch (className) {
          case 'page-btn':
            curPage = parseInt((<HTMLElement>tar).getAttribute('data-page')!);
    ...
    makeBtns: function (start: number, end: number, curPage: number) {
      var list = '';
    ...
    pageTpl: function (type: string, num: number, cur: number, pages: number) {
      switch (type) {
        case 'btn':
          return num === cur ?
    ...
    renderPageList: function (curPage: number, pages: number) {
      if (pages <= 0) {
        return '';
      }
