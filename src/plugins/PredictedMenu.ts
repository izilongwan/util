...
import { addEvent, removeEvent } from '../element/event'
...
export const PredictedMenu = (function (win, doc) {
	var PredictedMenu = function (this: ICommonObject, wrap: string, opt: ICommonObject = {}) {
		this.oWrap = doc.querySelector(wrap);
		this.oMainMenu = this.oWrap.getElementsByClassName(opt.mainMenu)[0];
		this.oMainItems = this.oWrap.getElementsByClassName(opt.mainItem) as HTMLElement[];
	...
	PredictedMenu.prototype = {
		...
		bindEvent: function () {
			...
			this.oMainItems._forEach(function (val: HTMLElement) {
		...
		enterMenu: function () {
			...
			addEvent(<HTMLElement><unknown>doc, 'mousemove', this._mouseMove);
		...
		enterMianMenu: function (this: ICommonObject, e: MouseEvent) {
			var e = e || win.event,
					tar = (e.target || e.srcElement) as HTMLElement,
					idx = ([].indexOf as any).call(this.oMainItems, tar),
		...
		doTimeout: function (cur: IVecType, last: IVecType) {
		...
		leaveMenu: function () {
			...
			removeEvent(<HTMLElement><unknown>doc, 'mousemove', this._mouseMove);
		...
		showMenuItem: function (idx: number) {
			this.restoreMenuItems();
			if (!this.isLeave) {
		...
		restoreMenuItems: function () {
			var _self = this;
			this.oMainItems.forEach(function (val: HTMLElement) {
				val.classList.remove(_self.mainItemCurrent);
			});
			this.oSubItems.forEach(function (val: HTMLElement) {
